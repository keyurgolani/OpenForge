...
function AudioWaveformPlayer({ fileUrl, duration, format }: { fileUrl: string; duration?: number; format?: string }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-border/20 bg-violet-500/5 py-5 px-4" onClick={e => e.stopPropagation()} onPointerDown={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    {sizeLabel && (
                        <span className="text-[10px] text-muted-foreground/70 rounded-full border border-border/20 bg-muted/30 px-2 py-0.5">
                            {sizeLabel}
                        </span>
                    )}
                </div>
            ...
            {isProcessing && !hasThumbnail && !isAudio ? (
                ...
                <div className={`flex items-center justify-center rounded-lg border border-border/20 ${meta.bgColor} py-5`}>
