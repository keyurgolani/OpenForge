...
export function GistCard({ item, slim, isProcessing }: { item: KnowledgeListItem; slim?: boolean; isProcessing?: boolean }) {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            {/* Code snippet */}
            {codeLines && (
                <pre className="text-[11px] font-mono whitespace-pre-wrap text-foreground/80 bg-muted/25 rounded-lg px-3 py-2 overflow-hidden max-h-[7.5rem] border border-border/20">
                    {codeLines}
                </pre>
            )}
