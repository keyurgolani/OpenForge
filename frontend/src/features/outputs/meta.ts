import type { Output, OutputCreationMode, OutputLink, OutputObjectType, OutputVisibility } from '@/types/outputs'
import type { ArtifactType } from '@/types/common'

export const OUTPUT_TYPE_META: Record<ArtifactType, { label: string; accent: string }> = {
...
const VISIBILITY_LABELS: Record<OutputVisibility, string> = {
  private: 'Private',
  workspace: 'Workspace',
...
export function getOutputTypeLabel(type: ArtifactType): string {
  return OUTPUT_TYPE_META[type]?.label ?? type
}

export function getOutputTypeAccent(type: ArtifactType): string {
  return OUTPUT_TYPE_META[type]?.accent ?? OUTPUT_TYPE_META.other.accent
}

export function getOutputVisibilityLabel(visibility: OutputVisibility): string {
  return VISIBILITY_LABELS[visibility] ?? visibility
}

export function getOutputOriginLabel(creationMode: OutputCreationMode): string {
  return ORIGIN_LABELS[creationMode] ?? creationMode
}

export function getOutputLinkTargetLabel(targetType: OutputObjectType): string {
...
export function getOutputLinkHref(link: OutputLink): string | null {
