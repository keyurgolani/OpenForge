...
import { getArtifactLinkHref, getArtifactLinkTargetLabel } from '@/features/artifacts/meta'
            ...
            const content = (
              <div className="rounded-xl border border-border/50 bg-background/35 px-3 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-foreground">{item.label || getArtifactLinkTargetLabel(item.target_type)}</p>
                    <p className="mt-1 text-xs text-muted-foreground/80">
                      {getArtifactLinkTargetLabel(item.target_type)} · {item.target_id}
                    </p>
                  </div>
  ...
  return (
    <Card glass>
      <CardHeader>
        ...
        <CardDescription>Make the artifact’s origin visible instead of hiding it in metadata blobs.</CardDescription>
      </CardHeader>
