...
import {
  ...
  getArtifactOriginLabel,
  getArtifactSourceChips,
  getArtifactTypeAccent,
  getArtifactTypeLabel,
  getArtifactVisibilityLabel,
  stringifyStructuredPayload,
  useArtifactLineageQuery,
  useArtifactQuery,
  useArtifactSinksQuery,
  useArtifactVersionDiffQuery,
  useArtifactVersionsQuery,
} from '@/features/artifacts'
import { addArtifactSink, createArtifactVersion, deleteArtifact, promoteArtifactVersion, updateArtifact } from '@/lib/api'
import { artifactsRoute } from '@/lib/routes'
import type { ArtifactSinkType, ArtifactStatus, ArtifactVersion, ArtifactVisibility } from '@/types/artifacts'
...
export default function ArtifactDetailPage() {
  ...
  const { data: artifact, isLoading, error } = useArtifactQuery(artifactId)
  const { data: versionsData } = useArtifactVersionsQuery(artifactId)
  const { data: lineage } = useArtifactLineageQuery(artifactId)
  const { data: sinksData } = useArtifactSinksQuery(artifactId)
  ...
  const [sinkDraft, setSinkDraft] = useState<{ sink_type: ArtifactSinkType; destination_ref: string }>({
  ...
  useEffect(() => {
    ...
    setVersionDraft({
      body: artifact.current_version?.content ?? String(artifact.content?.body ?? ''),
      structuredPayload: stringifyStructuredPayload(artifact.current_version?.structured_payload),
      changeNote: '',
    })
  }, [artifact])
  ...
  useEffect(() => {
    ...
    if (!selectedVersionId) {
      setSelectedVersionId(artifact.current_version?.id ?? versions[0]?.id)
      return
    }
  ...
  const selectedVersion = useMemo(
    () => versions.find((version) => version.id === selectedVersionId) ?? versions[0] ?? artifact?.current_version ?? null,
    [artifact?.current_version, selectedVersionId, versions],
  )
  ...
  const diffQuery = useArtifactVersionDiffQuery(artifactId, selectedVersion?.id, compareVersion?.id)

  const invalidateArtifactQueries = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['artifacts'] }),
      queryClient.invalidateQueries({ queryKey: ['artifact', artifactId] }),
      queryClient.invalidateQueries({ queryKey: ['artifact', artifactId, 'versions'] }),
      queryClient.invalidateQueries({ queryKey: ['artifact', artifactId, 'lineage'] }),
      queryClient.invalidateQueries({ queryKey: ['artifact', artifactId, 'sinks'] }),
      queryClient.invalidateQueries({ queryKey: ['artifact', artifactId, 'diff'] }),
    ])
  }

  const saveMetadataMutation = useMutation({
    mutationFn: async () => updateArtifact(artifactId, {
      title: metadataDraft.title,
      summary: metadataDraft.summary || undefined,
      status: metadataDraft.status,
      visibility: metadataDraft.visibility,
    }),
    onSuccess: invalidateArtifactQueries,
  })

  const archiveMutation = useMutation({
    mutationFn: async () => updateArtifact(artifactId, { status: 'archived' }),
    onSuccess: invalidateArtifactQueries,
  })
  ...
  const createVersionMutation = useMutation({
    mutationFn: async () => {
      ...
      return createArtifactVersion(artifactId, {
        body: versionDraft.body,
        structured_payload: parsedPayload,
        content_type: 'markdown',
        summary: metadataDraft.summary || undefined,
        change_note: versionDraft.changeNote || 'New artifact version',
        status: metadataDraft.status,
      })
    },
  ...
  const promoteVersionMutation = useMutation({
    mutationFn: async (versionId: string) => promoteArtifactVersion(artifactId, versionId),
    onSuccess: async (updated) => {
      setSelectedVersionId(updated.current_version?.id ?? undefined)
      await invalidateArtifactQueries()
    },
  ...
  const addSinkMutation = useMutation({
    mutationFn: async () => addArtifactSink(artifactId, {
      sink_type: sinkDraft.sink_type,
      sink_state: 'configured',
      destination_ref: sinkDraft.destination_ref || undefined,
      sync_status: 'pending_sync',
      metadata: { added_from: 'artifact_detail_page' },
    }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['artifact', artifactId, 'sinks'] })
  ...
  if (error || !artifact) {
    return <ErrorState message="Artifact detail could not be loaded from the canonical artifact API." />
  }
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.85fr)]">
        <div className="space-y-6">
          <Section title="Artifact Identity" description="Artifact metadata can change in place; material body changes should create versions below.">
            <Card glass>
              <CardContent className="grid gap-4 md:grid-cols-2">
                ...
                <label className="space-y-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) => setMetadataDraft((current) => ({ ...current, status: event.target.value as ArtifactStatus }))}
                  >
                ...
                <label className="space-y-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) => setMetadataDraft((current) => ({ ...current, visibility: event.target.value as ArtifactVisibility }))}
                  >
                ...
                <div className="rounded-2xl border border-border/50 bg-background/35 p-4 text-sm text-muted-foreground/85">
                  <p className="font-medium text-foreground">{getArtifactOriginLabel(artifact.creation_mode)}</p>
          ...
          <Section title="Current View" description="The selected version preview stays separate from the metadata form so version state is visible and inspectable.">
            <Card glass>
              <CardHeader>
                <CardTitle as="h2">Version {selectedVersion?.version_number ?? artifact.current_version_number}</CardTitle>
              ...
              <CardContent className="space-y-4">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={renderMarkdown(selectedVersion?.content ?? String(artifact.content?.body ?? ''))}
                />
          ...
          <Section title="Create New Version" description="Use this form when the artifact body or structured payload materially changes.">
            <Card glass>
        ...
        <div className="space-y-6">
          <Card glass>
            <CardHeader>
              <CardTitle as="h2">Artifact Snapshot</CardTitle>
            ...
            <CardContent className="space-y-3 text-sm text-muted-foreground/85">
              <div className="flex items-center justify-between">
                ...
                <span className="text-foreground">v{artifact.current_version_number}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Origin</span>
                <span className="text-foreground">{getArtifactOriginLabel(artifact.creation_mode)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Visibility</span>
                <span className="text-foreground">{getArtifactVisibilityLabel(artifact.visibility)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Sinks</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-foreground">{artifact.updated_at ? new Date(artifact.updated_at).toLocaleString() : 'Unknown'}</span>
              </div>
              ...
              <div className="grid gap-3 rounded-xl border border-border/50 bg-card/40 p-4">
                ...
                <label className="space-y-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) => setSinkDraft((current) => ({ ...current, sink_type: event.target.value as ArtifactSinkType }))}
                  >
                ...
                <label className="space-y-2 text-sm">
                  ...
                  <input
                    ...
                    placeholder="export://artifact.md or external://placeholder"
                  />
                </label>
                <MutationButton
                  type="button"
