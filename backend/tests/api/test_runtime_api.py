...
class WorkflowRuntimeStubService:
...
def create_client() -> TestClient:
    ...
    app.dependency_overrides[get_workflow_service] = lambda: WorkflowRuntimeStubService()
    app.dependency_overrides[get_run_service] = lambda: RunRuntimeStubService()
    return TestClient(app)


def test_workflow_versions_endpoint_returns_expected_shape() -> None:
...
def test_run_steps_checkpoint_and_event_endpoints_return_runtime_data() -> None:
    client = create_client()
    run_id = str(uuid4())
