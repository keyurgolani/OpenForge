...
class ArtifactService:
    ...
    async def _build_artifact_response(self, artifact: ArtifactModel) -> dict[str, Any]:
    ...
    async def list_artifacts(
        ...
        artifacts = []
        for row in rows:
            artifacts.append(await self._build_artifact_response(row))
        return artifacts, int(total or 0)

    async def get_artifact(self, artifact_id: UUID) -> dict[str, Any] | None:
        artifact = await self.db.get(ArtifactModel, artifact_id)
        if artifact is None:
            return None
        return await self._build_artifact_response(artifact)

    async def create_artifact(self, artifact_data: dict[str, Any]) -> dict[str, Any]:
        version_payload = self._normalize_version_payload(artifact_data)
        artifact = ArtifactModel(
            artifact_type=self._value(artifact_data["artifact_type"]),
            workspace_id=artifact_data["workspace_id"],
            source_run_id=artifact_data.get("source_run_id"),
            source_workflow_id=artifact_data.get("source_workflow_id"),
            source_mission_id=artifact_data.get("source_mission_id"),
            source_profile_id=artifact_data.get("source_profile_id"),
            title=artifact_data["title"],
            summary=artifact_data.get("summary"),
            content=self._build_legacy_content(version_payload),
            metadata_json=artifact_data.get("metadata", {}),
            status=self._value(artifact_data.get("status", "draft")),
            visibility=self._value(artifact_data.get("visibility", "workspace")),
            creation_mode=self._value(artifact_data.get("creation_mode", "user_created")),
            version=0,
            created_by_type=artifact_data.get("created_by_type"),
            created_by_id=artifact_data.get("created_by_id"),
            tags_json=artifact_data.get("tags", []),
        )
        ...
        for link_payload in default_links + list(artifact_data.get("links", [])):
            await self._create_link(artifact.id, version.id, link_payload)

        sink_payloads = list(artifact_data.get("sinks", [])) or [build_default_sink()]
    ...
    async def update_artifact(self, artifact_id: UUID, artifact_data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if artifact_data.get("title") is not None:
            artifact.title = artifact_data["title"]
        if artifact_data.get("summary") is not None and not should_create_new_version(artifact_data):
            artifact.summary = artifact_data["summary"]
        if artifact_data.get("metadata") is not None:
            artifact.metadata_json = artifact_data["metadata"]
        if artifact_data.get("status") is not None:
            artifact.status = self._value(artifact_data["status"])
        if artifact_data.get("visibility") is not None:
            artifact.visibility = self._value(artifact_data["visibility"])
        if artifact_data.get("tags") is not None:
            artifact.tags_json = artifact_data["tags"]

        promote_version_id = artifact_data.get("promote_version_id")
        if promote_version_id is not None:
            version = await self.db.get(ArtifactVersionModel, promote_version_id)
            if version is not None and version.artifact_id == artifact_id:
                artifact.current_version_id = version.id
                artifact.version = version.version_number
        ...
        if should_create_new_version(artifact_data):
            version_payload = self._normalize_version_payload(artifact_data)
    ...
    async def create_version(self, artifact_id: UUID, version_data: dict[str, Any]) -> dict[str, Any] | None:
        return await self.update_artifact(artifact_id, version_data)

    async def promote_version(self, artifact_id: UUID, version_id: UUID) -> dict[str, Any] | None:
        return await self.update_artifact(artifact_id, {"promote_version_id": version_id})
