...
// Intercept 401 responses and dispatch a custom event so the auth guard can react
...
export const downloadTTSModel = (modelId: string): Promise<any> =>
  api.post('/models/tts/download', { model_id: modelId }).then((r) => r.data)
export const deleteTTSModel = (modelId: string): Promise<any> =>
  api.delete(`/models/tts/${modelId}`).then((r) => r.data)
export const getTTSDefault = (): Promise<any> => api.get('/models/tts/default').then((r) => r.data)
export const setTTSDefault = (modelId: string): Promise<any> =>
  api.put('/models/tts/default', { model_id: modelId }).then((r) => r.data)
...
export const mergeWorkspaces = (targetId: string, sourceId: string, deleteSource = true): Promise<any> =>
  api
    .post(`/workspaces/${targetId}/merge`, { source_workspace_id: sourceId, delete_source: deleteSource })
    .then((r) => r.data)
...
export const uploadKnowledge = (wid: string, file: File): Promise<any> => {
  ...
  return api
    .post(`/workspaces/${wid}/knowledge/upload`, formData, {
...
export const visualSearch = (wid: string, file: File, limit?: number): Promise<any> => {
  ...
  formData.append('file', file)
  return api
    .post(`/workspaces/${wid}/knowledge/search/visual`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
...
): Promise<any> => api.put(`/tasks/schedules/${id}`, data).then((r) => r.data)
export const runTaskNow = (
  id: string,
  data?: { target_scope?: 'one' | 'remaining' | 'all'; workspace_id?: string; knowledge_id?: string },
): Promise<any> => api.post(`/tasks/schedules/${id}/run`, data ?? {}).then((r) => r.data)
export const getTaskHistory = (params?: {
  task_type?: string
  workspace_id?: string
  limit?: number
}): Promise<any> => api.get('/tasks/history', { params }).then((r) => r.data)
export const getToolCallLogs = (params?: {
  workspace_id?: string
  tool_name?: string
  limit?: number
}): Promise<any> => api.get('/tasks/tool-call-logs', { params }).then((r) => r.data)
...
export const listToolPermissions = async (): Promise<any> => {
  ...
  return [
    ...
    ...(systemPolicy.approval_required_tools ?? []).map((toolId: string) => ({
      tool_id: toolId,
      permission: 'hitl',
    })),
...
export const listApprovalRequests = (params?: {
  ...
  limit?: number
  offset?: number
}): Promise<any> => api.get('/policies/approvals', { params }).then((r) => r.data)
...
}): Promise<{ runs: Run[]; total: number }> => api.get('/runs', { params }).then((r) => r.data)
...
export const getRunLineage = (id: string): Promise<RunLineage> =>
  api.get(`/runs/${id}/lineage`).then((r) => r.data)
export const getRunCompositeDebug = (id: string): Promise<RunCompositeDebug> =>
  api.get(`/runs/${id}/composite`).then((r) => r.data)
...
export const createOutput = (data: OutputCreate): Promise<Output> =>
  api.post('/outputs', data).then((r) => r.data)
export const updateOutput = (id: string, data: OutputUpdate): Promise<Output> =>
  api.patch(`/outputs/${id}`, data).then((r) => r.data)
export const deleteOutput = (id: string): Promise<void> => api.delete(`/outputs/${id}`) as any
...
export const getOutputVersionDiff = (
  ...
  api
    .get(`/outputs/${outputId}/versions/${versionId}/diff`, {
      params: { compare_to_version_id: compareToVersionId },
    })
    .then((r) => r.data)
...
export const listAgents = (params?: {
  skip?: number
  limit?: number
}): Promise<AgentDefinitionListResponse> => api.get('/agents', { params }).then((r) => r.data)
export const getAgent = (id: string): Promise<AgentDefinition> =>
  api.get(`/agents/${id}`).then((r) => r.data)
export const createAgent = (data: AgentDefinitionCreate): Promise<AgentDefinition> =>
  api.post('/agents', data).then((r) => r.data)
export const updateAgent = (id: string, data: AgentDefinitionUpdate): Promise<AgentDefinition> =>
...
): Promise<AgentDefinitionVersionListResponse> =>
...
export const listAutomations = (params?: {
  ...
  skip?: number
  limit?: number
}): Promise<any> => api.get('/automations', { params }).then((r) => r.data)
export const getAutomation = (id: string): Promise<any> =>
  api.get(`/automations/${id}`).then((r) => r.data)
export const createAutomation = (data: AutomationCreate): Promise<any> =>
  api.post('/automations', data).then((r) => r.data)
export const updateAutomation = (id: string, data: AutomationUpdate): Promise<any> =>
  api.patch(`/automations/${id}`, data).then((r) => r.data)
export const deleteAutomation = (id: string) => api.delete(`/automations/${id}`)
export const compileAutomation = (id: string): Promise<any> =>
  api.post(`/automations/${id}/compile`).then((r) => r.data)
export const pauseAutomation = (id: string): Promise<any> =>
  api.post(`/automations/${id}/pause`).then((r) => r.data)
export const resumeAutomation = (id: string): Promise<any> =>
  api.post(`/automations/${id}/resume`).then((r) => r.data)
export const activateAutomation = (id: string): Promise<any> =>
  api.post(`/automations/${id}/activate`).then((r) => r.data)
export const getAutomationHealth = (id: string): Promise<any> =>
  api.get(`/automations/${id}/health`).then((r) => r.data)
...
export const saveAutomationGraph = async (
  id: string,
  graph: { nodes: unknown[]; edges: unknown[]; static_inputs: unknown[] },
...
export const deployAutomation = async (
  automationId: string,
  data: { workspace_id: string; input_values: Record<string, unknown>; schedule_expression?: string },
) => (await api.post(`/automations/${automationId}/deploy`, data)).data
export const listDeployments = async (params?: {
  status?: string
  automation_id?: string
  workspace_id?: string
  skip?: number
...
export const getDeployment = async (id: string) => (await api.get(`/deployments/${id}`)).data
...
// -- Global Chat ----------------------------------------------------------------
export const listGlobalConversations = async (params?: {
  agent_id?: string
  skip?: number
  limit?: number
  category?: string
...
export const updateGlobalConversation = async (
  id: string,
  data: { title?: string; title_locked?: boolean; is_pinned?: boolean; is_archived?: boolean },
...
export const addGlobalMessage = async (
  conversationId: string,
  data: { content: string; role?: string; model_id?: string },
...
export const exportGlobalConversation = (cid: string, format = 'json'): Promise<Blob> =>
  api
    .get(`/chat/conversations/${cid}/export`, { params: { format }, responseType: 'blob' })
    .then((r) => r.data)

// -- Template Engine ------------------------------------------------------------
export const getTemplateReference = async () => (await api.get('/template-engine/reference')).data
