...
export default function ReasoningPage() {
  ...
  useEffect(() => {
    if (settingsQuery.data) {
      ...
      const reasoningSetting = Array.isArray(settings)
        ? settings.find((s: any) => s.key === 'reasoning_model')
        : settings.reasoning_model
      if (reasoningSetting) {
        setSelectedModel(reasoningSetting.value ?? reasoningSetting ?? '')
      }
    }
  }, [settingsQuery.data])
  ...
  const saveMut = useMutation({
    mutationFn: () =>
      updateSetting('reasoning_model', { value: selectedModel, category: 'models' }),
    onSuccess: () => {
      toast.success('Reasoning model updated')
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-display text-base font-semibold text-fg">Reasoning Model</h3>
        <p className="text-sm text-fg-muted">
          Select the primary model used for complex reasoning tasks, planning, and multi-step
          analysis by agents.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-border/40 bg-bg-elevated p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 space-y-4">
            <div>
              <h4 className="font-label text-sm font-medium text-fg">Primary Reasoning Model</h4>
              <p className="mt-0.5 text-xs text-fg-muted">
                This model handles complex reasoning chains, agent planning, and analytical tasks.
